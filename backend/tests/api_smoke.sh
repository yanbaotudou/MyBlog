...
echo "[1/15] Register user: $USER_NAME"
...
echo "[6/15] Create collection and add posts"
...
echo "[7/15] Verify collection detail and prev/next navigation"
...
echo "[8/15] Remove one post from collection and verify reorder"
...
echo "[11/15] Change password and verify old password invalid"
...
echo "[12/15] Login with new password"
...
echo "[14/15] Normal user tries admin API (expect 403)"
STATUS=$(curl -sS -o /tmp/admin_forbidden.json -w '%{http_code}' "$BASE_URL/api/admin/users" \
  -H "Authorization: Bearer $USER_TOKEN")
[ "$STATUS" = "403" ]

echo "[15/15] Admin updates user role and ban, then soft delete posts"
